...
func (d *SuperDirectoryImpl) Lookup(bankID int, PID vm.PID, reqAddr uint64) (*CohEntry, int) {
	...
	for _, entry := range set.CohEntries {
		if entry.IsValid && entry.Tag>>maskLen == reqAddr>>maskLen && entry.PID == PID {
			...
			// MGD strict semantics: bank 0 is the coarse-grain region. The
			// entire region holds a single (flat) sharer set, mirroring
			// Zebchuk MICRO 2013 §3 region-entry layout. Sub-entries still
			// track per-block presence via IsValid, but their Sharer slices
			// are kept synchronized to the region-level union before any
			// permission / invalidation decision reads them. This is what
			// produces region-grained false invalidations.
			if bankID == 0 {
				syncFlatSharer(entry)
			}
			return entry, index
		}
	}
...
// syncFlatSharer rewrites every valid sub-entry's Sharer to the union of
// all valid sub-entries' Sharer values. Called only on bank-0 (coarse-region)
// entries — bank 1+ keeps independent sub-entry sharers.
func syncFlatSharer(entry *CohEntry) {
	seen := map[sim.RemotePort]bool{}
	union := []sim.RemotePort{}
	for i := range entry.SubEntry {
		if !entry.SubEntry[i].IsValid {
			continue
		}
		for _, sh := range entry.SubEntry[i].Sharer {
			if !seen[sh] {
				seen[sh] = true
	...
	for i := range entry.SubEntry {
		...
		out := make([]sim.RemotePort, len(union))
		copy(out, union)
		entry.SubEntry[i].Sharer = out
	}
}
