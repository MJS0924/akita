...
func (m *mshrImpl) Add(pid vm.PID, addr uint64, regionLen uint64, regionID int) *MSHREntry {
	for i, e := range m.entries {
		mask := max(e.RegionLen, regionLen)
		if e.PID == pid && e.Address>>mask == addr>>mask {
			fmt.Printf("[ERROR]\tentry already in mshr: input(addr %x regionLen %d), entry[%d](%v)\n",
				addr, regionLen, i, e)

			panic(fmt.Sprintf("entry already in mshr: input(addr %x regionLen %d), entry(%v)\n",
				addr, regionLen, e))
		}
	}
