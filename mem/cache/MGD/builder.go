...
import (
	...
	"github.com/sarchlab/akita/v4/mem/cache/MGD/internal"
...
// MakeBuilder creates a new builder with default configurations for MGD
// (Multi-Grain Directory, Zebchuk MICRO 2013). The default region size is
// 1KB; override via WithRegionSize. MGD is single-banked at the region grain;
// Bank 0 enforces flat (region-level) sharer semantics, which is the source
// of the false-invalidation behavior the MGD baseline must exhibit.
func MakeBuilder() Builder {
	return Builder{
		...
		log2BlockSize:       6,
		numBanks:            1,
		log2NumSubEntry:     4, // 1KB region default: 16 × 64B sub-blocks
		byteSize:            512 * mem.KB,
		numMSHREntry:        16,
		numReqPerCycle:      1,
...
// WithRegionSize sets the coarse-grain region size in bytes (e.g. 1024,
// 4096, 8192) and reconfigures the directory as a single coarse bank with
// per-block presence sub-entries. The value must be a power of two and an
// integer multiple of the cache block size.
func (b Builder) WithRegionSize(bytes uint64) Builder {
	blockSize := uint64(1) << b.log2BlockSize
	if bytes < blockSize {
		panic(fmt.Sprintf(
			"MGD: region size %d must be >= block size %d",
			bytes, blockSize))
	}
	if bytes%blockSize != 0 {
		panic(fmt.Sprintf(
			"MGD: region size %d must be a multiple of block size %d",
			bytes, blockSize))
	}
	ratio := bytes / blockSize
	if ratio&(ratio-1) != 0 {
		panic(fmt.Sprintf(
			"MGD: region/block ratio %d must be a power of two", ratio))
	}
	log2 := uint64(0)
	for r := ratio; r > 1; r >>= 1 {
		log2++
