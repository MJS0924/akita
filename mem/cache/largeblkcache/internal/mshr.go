...
func NewMSHR(capacity int, log2BlkSize uint64) MSHR {
	m := new(mshrImpl)
	m.capacity = capacity
...
func (m *mshrImpl) Add(pid vm.PID, addr uint64) *MSHREntry {
	for _, e := range m.entries {
		if e.PID == pid && e.Address>>m.log2BlkSize == addr>>m.log2BlkSize {
			panic(fmt.Sprintf("entry already in mshr: Addr %x, PId %d", addr, pid))
		}
	}
	...
	entry := NewMSHREntry()
	entry.PID = pid
	entry.Address = addr >> m.log2BlkSize << m.log2BlkSize
